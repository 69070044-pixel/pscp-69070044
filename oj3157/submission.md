...
```text
ให้หาผลสุดท้ายของคะแนนที่ถูกเพิ่มและลด
ตอนแรกแรกรับ input เป็นจำนวนครั้งในการ input ว่าจะ "+" = +10 หรือ "-" = -5
```
...
```text
Step 1: รับ input จำนวนครั้งและสร้าง total เพื่อเก็บผลรวม
Step 2: ใช้ while loop เพื่อรับค่าจนครบ
Step 3: รับ input ช = input() == "+" จะได้ค่า True เมื่อ user input "+"
Step 4: check ว่า input == True มั้ยด้วย if ถ้าใช้ให้ total += 10 ถ้าไม่ total -= 5
Step 5: เมื่อจบ loop print(total)
```
...
```text
เหมือนแผนแรกเมื่อ input เป็น + ให้ total += 10 ถ้าไม่ก็ลดคะแนน total -= 5
```
