...
```text
input คือผลรวมคะแนนของ 3 คน และคะแนนที่มากที่สุดในทั้งสามคน โดยที่คะแนนเต็ม/คน คือ 10
โดยให้เราหาค่าที่น้อยที่สุดที่เป็นไปได้และนำมาเปรียบเทียบกับค่าที่มากที่สุดที่รับมา
หากห่างกันมากกว่า 2 คะแนน output จะเป็น Surprising แต่ถ้าไม่ให้แสดง Not surprising
```
...
Step 2:นำค่าที่รับมาหาค่า min
Step 3:นำไปเปรียบเทียบค่า
Step 4:แสดงผลคำตอบ
```
...
```text
เหมือนกันกับวิธีแรกโดยเพิ่มเงื่อนไขก่อนเปรียบเทียบค่าว่า
เมื่อ min < 0 ให้ min = 0
เนื่องจากบางกรณีอาจทำให้ค่าติดลบ
```
...
```text
ดิว นายธนสร นุชประมูล (69070079)
```

เขาช่วยอะไร

```text
แนะนำเกี่ยวกับสูตรที่ให้หาค่าต่ำสุด
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ดและเงื่อนไขเอง
```
