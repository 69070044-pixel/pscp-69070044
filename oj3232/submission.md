...
```text
นับจำนวนครั้งการกระโดดจนถึงระยะ y โดยที่แรกเริ่มจะกระโดยได้ระยะเท่ากับ x และระยะกระโดด x จะลดลง 2 ในทุกๆครั้งที่กระโดด
หาก x = 0 แระยังไปไม่ถึงระยะ y ให้แสดง -1 หากไปถึงได้ให้แสดงจำนวนครั้งการกระโดดทั้งหมด 
```
...
```text
Step 1: รับ input และประกาศตัวแปรที่ต้องใช้ goal, step
Step 2: ใช้ for i in range(0, x + 1, 2) เพื่อ loop แบบ step ทีละ 2 จาก 0 -> X + 1
Step 3: เก็บค่าระยะทางปัจจุบัน goal = goal + (x - i) และนับ step += 1 ไปด้วย
Step 4: ถ้า goal >= y แล้วให้หยุดทันที
Step 5: ใช้ if goal >= y อีกทีนอก loop เพื่อดูว่าไปถึง y มั้ยถ้าไม่ให้ print(-1) ถ้าใช่ print(step)
```
...
```text
เหมือนแผนแรกเมื่อ input ประกาศตัวแปรที่ต้องใช้และ ใช้ for loop นับทีละ step
```
