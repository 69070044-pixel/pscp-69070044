...
```text
input คือ string ที่เป็นคำหรือประโยคยาวๆ
output คือจำนวนของสระ a e i o u ที่นับได้ตามลำดับและมีจำนวน 1 ตัวขึ้นไปถึงจะแสดง
```
...
```text
Step 1:รับ input ซึ่งคือข้อความ string
Step 2:ใช้ for loop กับ list ที่เก็บ a e i o u ไว้เพื่อใช้ลูปสำหรับการนับ
Step 3:ใช้ for loop เพื่อ ไล่นับตัวอักษรที่ตรงกับใน list ที่อยู่ในข้อความโดยใช้ count
Step 4:ถ้านับได้มากกว่า 0 ให้แสดงผลตัวอักษรนั้นและจำนวนที่นับได้
```
...
```text
เหมือนกับแผนแรกเลยใช้ for loop กับ ['a', 'e', 'i', 'o', 'u'] และให้ไล่นับทีละสระด้วย .count()
```
