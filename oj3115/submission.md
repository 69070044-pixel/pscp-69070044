...
```text
ให้หาว่าในช่วงเวลาที่ต้องการ check นั้นมีร้านเปิดกี่ร้าน คือ และ input อยู่ในหน่วยนาทีทั้งหมด
output คือจำนวนร้านที่เป็นของแต่ละเวลา
```
...
```text
Step 1: รับ input ของจำนวนร้านและจำนวนเส้นทาง
Step 2: รับ input ของเวลาเปิดปิดแต่ละร้านในรูปแบบของ list ใน list ex. [[start1, stop1], [start2, stop2]]
Step 3: รับ input ของเวลาที่ไปของแต่ละเส้นทางโดยเก็บเป็น list
Step 4: check ว่า (เวลาเปิด<= เวลาที่ไป < เวลาปิด) เป็นจริงหรือไม่ถ้าใช่ก็นับบวกไป 1 ร้าน ด้วยการใช้ for loop ร่วมกับ list
Step 5: เมื่อจบ loop ของแต่ละเส้นทางให้แสดงผลแบบไม่ขึ้นบรรทัดใหม่และ loop ต่อในเส้นทางต่อไปทำแบบเดิมจนจบ
```
...
```text
เหมือนแผนแรกรับ input มาจัดเป็น list แล้วนำไปใช้ for loop ร่วมกับ list เพื่อ check ว่า
(เวลาเปิด<= เวลาที่ไป < เวลาปิด) เป็นจริงหรือไม่ถ้าใช่ก็นับบวกไป 1 
```
...
```text
ไม่มีร้านไหนเปิดและไปตอนร้านปิดพอดี
```
...
```text
ไประหว่างร้านเปิดและไปตอนร้านเปิดพอดี
```
