...
```text
OJ3386 - [LEARNING LOGS] Duplicate I
```
...
```text
input คือจำนวนนักเรียนของทั้ง 2 กลุ่มและตามด้วย id ของทุกคนในแต่ละกลุ่ม
output คือให้หา id ที่อยู่ในทั้งสองกลุ่มและถ้าไม่มีให้ print Nope
โดยที่หากมี id ที่ว้ำหลายรายการจะต้องแสดงผลแบบเรียนลำดับจากมากไปน้อย
```
...
```text
Step 1: รับค่าจำนวนคนในกลุ่มแต่ละกลุ่ม
Step 2: รับ id ของแต่ละคนจนครบโดยทั้งหมดจะถูกแยกเก็บใน set ของแต่ละกลุ่ม
Step 3: นำ set 2 อันมา intersection เพื่อจะได้ result ที่เป็นตัวว้ำของทั้งคู่
Step 4: นำไปแสดงผลตามที่โจทย์ต้องการ
```
...
```text
เหมือนแผนแรกใช้ set ในการเก็บ id และหา result ด้วย intersection
```
