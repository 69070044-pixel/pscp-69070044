...
```text
triangle = {
    17:18, 19:20, 21:20, 23:22,
    25:24, 18:10, 20:12, 22:14,
    24:16, 10:11, 12:13, 14:13,
    16:15, 11:5, 13:7, 15:9,
    5:6, 7:6, 9:8, 6:2, 8:4,
    2:3, 4:3, 3:1
}
...
current = start
while current != 1:
    current = triangle[current]
    wall_pass += 1
print(wall_pass)

จากโค้ด
Step 1: ด้วยความที่ดูจากรูปแล้วเลยคิดว่าอาจจะสร้างเป็น Dict ที่เอาเลขมาเชื่อมกันได้
...
โดยการใช้ while loop และเช็กเงื่อนไขตาม pattern ที่เจอ
กำหนดให้ Input = n, row = (math.ceil(math.sqrt(n)))
โดยนำไปตรวจสอบว่าห้องและแถวที่อยู่เป็นเลขคี่หรือคู่เพื่อจะนำไป
ตรวจว่าควรจะไปที่ห้องเลขไหนต่อและเมื่อจบลูป 1 ครั้งก็เท่ากับพังไป 1 กำแพง
```
