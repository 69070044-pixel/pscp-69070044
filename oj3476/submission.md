...
```text
OJ3476 - [LEARNING LOGS] CuteCat CuteFox
```
...
```text
input1 คือจำนวนครั้งการรับข้อมูล
input2-n คือ ชื่อและประเภทว่าเป็น cat or fox ตามด้วยเลขลำดับเป็นรูปแบบ dict ex. {"Alex": "Cat01"}
โดยปกติในฝูงแมวจะมีจิ้งจอกและแมวอย่างละ 1 ตัวซึ่งเป็นหมายเลขหนึ่งคือ แมว(Cat01)จะชื่อว่า Garfield จิ้งจอก(Fox01)จะชื่อว่า Fubuki (เช่น ไม่มีการกำหนด Cat01 หรือ Fox01 ก็จะให้เป็นตามข้างต้นนั้นเอง)
output จำนวนของ cat and fox in dict และรายชื่อโดยเรียงตามลำดับตัวเลขจากน้อไปมาก
```
...
```text
Step 1: รับค่าจำนวนครั้ง
Step 2: รับข้อมูลที่จะเพิ่มเข้า dict ด้วย json จำนวน n ครั้ง
Step 3: เพิ่มค่า Cat01 and Fox01 เข้า Dictหากไม่มีการจำหนดใน input
Step 4: count Cat and Fox
Step 5: convert dict to list for sort and print
```
...
```text
เหมือนแผนแรกใช้ json ในการช่วยแปลง string input เป็น dict เพิ่มค่าด้วย dict.update(data)
count cat and fox then sort and print
```
