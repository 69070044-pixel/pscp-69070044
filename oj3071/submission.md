...
```text
ให้หาจำนวนของตัวเลขที่อยู่ในช่วง a -> b ที่หาร d แล้วได้เศษเท่า r ว่ามีทั้งหมดกี่ตัวใน range(a, b)
```
...
```text
Step 1:รับ input ทั้งหมดแล้วสร้างตัวแปรไว้เก็บจำนวนที่นับ
Step 2:ใช้ for i in range(a, b + 1) loop เพื่อจะนับไปเรื่อยจาก a -> b
Step 3:และเมื่อ i % d == r ให้เพิ่มจำนวนที่นับได้ +1 
Step 4:เมื่อจบ loop ให้แสดงผลข้อความ
```
...
```text
เหมือนกับแผนแรกเลยใช้ for loop นับบวกทีละ 1 เมื่อ i % d == r
```
...
```text
เลขที่หารตัวเองซึ่งจะไม่มีเศษและให้ d = 0
```
...
```text
ไม่มีตัวไหนไหน a -> b ที่หารด้วย d แล้วได้เศษ r
```
