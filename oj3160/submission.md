...
```text
ให้รับ input สองจำนวนและ output เป็น prime number ที่อยู่ใน range นั้น
และจำนวนของ prime number ทั้งหมดว่ามีกี่ตัว
```
...
```text
Step 1: define funcion สำหรับตรวจ prime number โดย return boolean
Step 2: รับ input range ที่ต้องการ
Step 3: create prime number list ด้วยการใช้ [x for x in range(a, b + 1) if is_prime(x)]
Step 4: แสดงผลตามที่โจทย์กำหนด
```
...
```text
เหมือนแผนแรกเมื่อ input จะนำค่า a, b ไปสร้างเป็น list ของ prime number และนำไปใช้แสดงผลต่อ

[x for x in range(a, b + 1) if is_prime(x)] = จาก a ถึง b + 1 ถ้า is_prime(x) เป็จจริงให้เพิ่มค่าใน list
```
