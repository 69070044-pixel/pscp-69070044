...
```text
ให้หาว่าตั้งแต่เริ่มจนวนจบมีคนกี่คนที่ได้ดูของขวัญโดยจะหยุดเมื่อ ของขวัญวนไปทีละ K คนและอยู่กับคนที่ 1 or T(ขโมย)
แสดงผลคือจำนวนคนที่ได้ดูของขวัญแบบไม่นับคนเดิมซ้ำ
```
...
```text
Step 1: รับค่า n, k, t
Step 2: สร้าง list ของ numbers ตั้งแต่ 1 ถึง n
Step 3: กำหนด looked = [1] คือคนที่ 1 ได้ดูก่อนคนแรกเสมอ และ local_num = 1 เริ่มที่คนที่ 1
Step 4: loop ด้วย while คำนวณ index ใหม่จากตำแหน่งปัจจุบัน + k - 1 ใช้ % เพื่อให้ index วนกลับเมื่อเกิน list index
Step 5: อัปเดต local_num เป็นค่าตำแหน่งใหม่
Step 6: ถ้ายังไม่มี local_num ใน looked ให้เพิ่มเข้าไป 
Step 7: ถ้า local_num เป็น 1 หรือ t ให้หยุด loop
Step 8: แสดงจำนวนค่าที่อยู่ใน looked
```
...
```text
คล้ายๆกับแผนแรกใช้การวนค่าใน list ด้วย while loop และเพิ่มค่าใน looked ที่เปลี่ยเป็น set แทนเพื่อลดเวลาในการคำนวน
เมื่อ check if local_num in (1, t) เมื่อเป็นจริงให้ทำการ break ออกจาก loop และ แสดงผล len(looked)
```
