...
| OJ submission ID, if submitted | 594037 |
| OJ status | Pass |
...
```text
ในข้อนี้ผมใช้แผนนแรกที่คิดแล้วส่งพบว่ามี Testcase ที่ติด Timeout จึกให้ AI ช่วยดูว่าสามารถ optimize ตรงไหนได้บ้างที่จะทำให้มันเร็วขึ้นเพราะมั่นใจว่าวิธีที่ใช้นั้นถูกแล้วติดแค่เวลาที่ใช้มากกว่า 1s เท่านั้น
```
...
```text
จุดที่สามารถทำให้เร็วขึ้นได้เช่นการเปลี่ยน list เป็น set หรือ การใช้ list.index(x) นั้นทำให้เสียเวลาเพราะการทำงานคือไล่ดูทีละตัว
```
...
```text
code ในจุดที่สามารถแก้ไขได้เพื่อให้กรทำงานของ program ทำได้เร็วขึ้นและไม่เปลี่ยนแผนเดิม
```
...
```text
การทำงานของ .index()
การใช้ set เร็วกว่าการใช้ if x in list เพราะ set จะไม่เก็บค่าที่ซ้ำกันอยู่แล้ว
```
